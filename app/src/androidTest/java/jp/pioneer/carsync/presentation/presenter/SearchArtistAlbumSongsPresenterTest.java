...
/**
 * アーティスト指定アルバム内楽曲リスト画面（検索用）のpresenterテスト
 */
public class SearchArtistAlbumSongsPresenterTest {
    @Rule public MockitoRule mMockito = MockitoJUnit.rule();
    @InjectMocks SearchArtistAlbumSongsPresenter mPresenter = new SearchArtistAlbumSongsPresenter();
    ...
    public void testSetLoaderManager() throws Exception {
        ...
        verify(mockLoader).initLoader(eq(3), any(Bundle.class), any(SearchArtistAlbumSongsPresenter.class));
    }
    ...
    public void testOnArtistAlbumSongPlayAction() throws Exception {
        ...
        mPresenter.setArguments(params.toBundle());
        mPresenter.takeView(mView);
        mPresenter.onArtistAlbumSongPlayAction(3);
