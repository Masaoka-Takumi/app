...
public class ContactsFragmentTest {
    ...
    @Rule public FragmentTestRule<ContactsFragment> mFragmentRule = new FragmentTestRule<ContactsFragment>() {
        @Override
        protected ContactsFragment createDialogFragment() {
            return ContactsFragment.newInstance(Bundle.EMPTY);
        }
    };
    @Mock ComponentFactory mComponentFactory;
    @Mock ContactsPresenter mPresenter;
    /** 仮想的に作成するDBのカラム名 */
    ...
    public interface TestAppComponent extends AppComponent {
        ContactsFragmentTest.TestPresenterComponent presenterComponent(ContactsFragmentTest.TestPresenterModule module);
    }

    @PresenterLifeCycle
    @Subcomponent(modules = ContactsFragmentTest.TestPresenterModule.class)
    ...
    public void setUp() throws Exception {
        ...
        ContactsFragmentTest.TestAppComponent appComponent = DaggerContactsFragmentTest_TestAppComponent.builder().build();
        testApp.setAppComponent(appComponent);
        testApp.setComponentFactory(mComponentFactory);
        ContactsFragmentTest.TestPresenterComponent presenterComponent = appComponent.presenterComponent(new ContactsFragmentTest.TestPresenterModule());
        FragmentComponent fragmentComponent = presenterComponent.fragmentComponent(new FragmentModule());
        when(mComponentFactory.getPresenterComponent(appComponent, ContactsFragment.class)).thenReturn(presenterComponent);
        when(mComponentFactory.createFragmentComponent(any(ContactsFragmentTest.TestPresenterComponent.class), any(ContactsFragment.class))).thenReturn(fragmentComponent);

        // DBを仮想的に作成して値を追加していく。FROM配列の長さ＝addRowする配列の長さ。
        mCursor.addRow(new String[]{"1", "satou", "satousan", "", "0", "11122223333", "1",});
        mCursor.addRow(new String[]{"2", "suzuki", "suzukisan", "", "1", "44455556666", "2",});
        mCursor.addRow(new String[]{"3", "tanaka", "tanakasan", "", "0", "77777777777", "0",});
        mCursor.moveToFirst();
    }
    ...
    public void testNewInstance() throws Exception {
        ...
        final ContactsFragment fragment = mFragmentRule.getFragment();
        Instrumentation instr = InstrumentationRegistry.getInstrumentation();
        instr.runOnMainSync(() -> {
            Bundle args = new Bundle();
            ContactsFragment m_fragment;
     ...
     */
    @Test
    public void testOnClickFavorites() throws Exception {
        mFragmentRule.launchActivity(null);
        final ContactsFragment fragment = mFragmentRule.getFragment();
        Instrumentation instr = InstrumentationRegistry.getInstrumentation();
        ...
        onData(anything())
                ...
                .atPosition(0).onChildView(withId(R.id.check_icon)).perform(click());
        verify(mPresenter).onFavoritesAction(mCursor);
    }
    ...
    public void testSetGroupCursor() throws Exception {
        ...
        //お気に入りアイコンのチェック
        onData(anything()).inAdapterView(withId(R.id.list_view))
                .atPosition(0).onChildView(withId(R.id.check_icon)).check(matches(withDrawable(R.drawable.p0020_favoritebtn_1nrm)));
        onData(anything()).inAdapterView(withId(R.id.list_view))
                .atPosition(1).onChildView(withId(R.id.check_icon)).check(matches(withDrawable(R.drawable.p0020_favoritebtn_1nrm)));
        onData(anything()).inAdapterView(withId(R.id.list_view))
                .atPosition(2).onChildView(withId(R.id.check_icon)).check(matches(withDrawable(R.drawable.p0020_favoritebtn_1nrm)));
