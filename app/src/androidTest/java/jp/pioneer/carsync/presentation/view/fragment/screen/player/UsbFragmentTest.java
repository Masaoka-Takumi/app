 ...
 */
public class UsbFragmentTest {
    @Rule public MockitoRule mMockito = MockitoJUnit.rule();
    @Rule public FragmentTestRule<UsbFragment> mFragmentRule = new FragmentTestRule<UsbFragment>() {
        private UsbFragment mFragment;

        @Override
        protected UsbFragment createDialogFragment() {
            // 非UIスレッドから呼ばれるが、GestureDetectorがUIスレッドでインスタンス化しないとだめ
            Instrumentation instr = InstrumentationRegistry.getInstrumentation();
            instr.runOnMainSync(() -> mFragment = new UsbFragment());
            return mFragment;
        }
    };
    @Mock ComponentFactory mComponentFactory;
    @Mock UsbPresenter mPresenter;
    ...
    public interface TestAppComponent extends AppComponent {
        UsbFragmentTest.TestPresenterComponent presenterComponent(UsbFragmentTest.TestPresenterModule module);
    }

    @PresenterLifeCycle
    @Subcomponent(modules = UsbFragmentTest.TestPresenterModule.class)
    ...
    public class TestPresenterModule {
        ...
        @Provides
        public UsbPresenter provideContactsPresenter() {
            return mPresenter;
        }
    }
    ...
    public void setUp() throws Exception {
        ...
        UsbFragmentTest.TestAppComponent appComponent = DaggerUsbFragmentTest_TestAppComponent.builder().build();
        testApp.setAppComponent(appComponent);
        testApp.setComponentFactory(mComponentFactory);
        UsbFragmentTest.TestPresenterComponent presenterComponent = appComponent.presenterComponent(new UsbFragmentTest.TestPresenterModule());
        FragmentComponent fragmentComponent = presenterComponent.fragmentComponent(new FragmentModule());
        when(mComponentFactory.getPresenterComponent(appComponent, UsbFragment.class)).thenReturn(presenterComponent);
        when(mComponentFactory.createFragmentComponent(any(UsbFragmentTest.TestPresenterComponent.class), any(UsbFragment.class))).thenReturn(fragmentComponent);
    ...
    public void testNewInstance() throws Exception {
        ...
        final UsbFragment fragment = mFragmentRule.getFragment();
        Instrumentation instr = InstrumentationRegistry.getInstrumentation();
        instr.runOnMainSync(() -> {
            Bundle args = new Bundle();
            UsbFragment m_fragment;
    ...
    public void testOnTouchJacketView() throws Exception {
        mFragmentRule.launchActivity(null);
        UsbMediaInfo info = new UsbMediaInfo();
        info.playbackMode = PlaybackMode.PLAY;
        when(mPresenter.getUsbMediaInfo()).thenReturn(info);
//        onView(withId(R.id.jacket_view)).perform(click());
    ...
    public void testSetRepeatImage() throws Exception {
        ...
        final UsbFragment fragment = mFragmentRule.getFragment();
        Instrumentation instr = InstrumentationRegistry.getInstrumentation();
        instr.runOnMainSync(() -> fragment.setRepeatImage(CarDeviceRepeatMode.OFF));
