...
/**
 * AUTO PI設定情報パケットプロセッサ.
 * <p>
 * AUTO PI設定情報応答と通知で使用する。
 */
public class AutoPiSettingInfoPacketProcessor {
     ...
     */
    public AutoPiSettingInfoPacketProcessor(@NonNull StatusHolder statusHolder) {
    ...
    public Boolean process(@NonNull IncomingPacket packet) {
        try {
            ...
            // D1:AUTO PI設定
            setting.autoPiSetting = (ubyteToInt(data[1]) == 0x01);

            setting.updateVersion();
            Timber.d("process() AutoPiSetting = " + setting.autoPiSetting);
            return Boolean.TRUE;
