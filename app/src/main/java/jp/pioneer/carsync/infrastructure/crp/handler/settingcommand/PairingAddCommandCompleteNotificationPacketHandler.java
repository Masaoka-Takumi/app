...
/**
 * ペアリング追加コマンド完了通知パケットハンドラ.
 */
public class PairingAddCommandCompleteNotificationPacketHandler extends AbstractPacketHandler {
     ...
     */
    public PairingAddCommandCompleteNotificationPacketHandler(@NonNull CarRemoteSession session) {
        super(session);
    ...
    protected OutgoingPacket doHandle(@NonNull IncomingPacket packet) throws Exception {
        try {
            ...
            CrpPairingAddCommandCompleteEvent ev = new CrpPairingAddCommandCompleteEvent(result, bdAddress);
            Timber.d("doHandle() " + ev);
            getSession().publishEvent(ev);
            return getPacketBuilder().createPairingAddCompleteNotificationResponse();
