...
/**
 * DISP OFF設定情報パケットプロセッサ.
 * <p>
 * DISP OFF設定情報応答と通知で使用する。
 */
public class DisplayOffSettingInfoPacketProcessor {
     ...
     */
    public DisplayOffSettingInfoPacketProcessor(@NonNull StatusHolder statusHolder) {
    ...
    public Boolean process(@NonNull IncomingPacket packet) {
        try {
            ...
            // D1:AUTO PI設定
            setting.displayOffSetting = (ubyteToInt(data[1]) == 0x01);

            setting.updateVersion();
            Timber.d("process() DisplayOffSetting = " + setting.displayOffSetting);
            return Boolean.TRUE;
