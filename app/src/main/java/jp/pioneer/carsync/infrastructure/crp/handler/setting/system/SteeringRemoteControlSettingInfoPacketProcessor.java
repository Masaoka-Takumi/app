...
/**
 * ステアリングリモコン設定情報パケットプロセッサ.
 * <p>
 * ステアリングリモコン設定情報応答と通知で使用する。
 */
public class SteeringRemoteControlSettingInfoPacketProcessor {
     ...
     */
    public SteeringRemoteControlSettingInfoPacketProcessor(@NonNull StatusHolder statusHolder) {
    ...
    public Boolean process(@NonNull IncomingPacket packet) {
        try {
            ...
            // D1:ステアリングリモコン設定
            setting.steeringRemoteControlSetting = SteeringRemoteControlSettingType.valueOf(data[1]);

            setting.updateVersion();
            Timber.d("process() SteeringRemoteControlSetting = " + setting.steeringRemoteControlSetting);
            return Boolean.TRUE;
