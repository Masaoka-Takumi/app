...
/**
 * KEY BRIGHTNESS設定情報（個別設定モデル用）パケットプロセッサ.
 * <p>
 * KEY BRIGHTNESS設定情報（個別設定モデル用）応答と通知で使用する。
 */
public class KeyBrightnessSettingInfoPacketProcessor {
     ...
     */
    public KeyBrightnessSettingInfoPacketProcessor(@NonNull StatusHolder statusHolder) {
    ...
    public Boolean process(@NonNull IncomingPacket packet) {
        try {
            ...
            BrightnessSetting setting = illuminationSetting.keyBrightnessSetting;
            // D1:最小値
            // D2:最大値
            // D3:現在の設定値
            ...
            Timber.d("process() KeyBrightnessSetting = " + setting);
            return Boolean.TRUE;
