...
/**
 * LOUDNESS設定情報パケットプロセッサ.
 * <p>
 * LOUDNESS設定情報応答と通知で使用する。
 */
public class LoudnessSettingInfoPacketProcessor {
     ...
     */
    public LoudnessSettingInfoPacketProcessor(@NonNull StatusHolder statusHolder) {
    ...
    public Boolean process(@NonNull IncomingPacket packet) {
        try {
            ...
            // D1:設定値
            audioSetting.loudnessSetting = LoudnessSetting.valueOf(data[1]);

            audioSetting.updateVersion();
            Timber.d("process() LoudnessSetting = " + audioSetting.loudnessSetting);
            return Boolean.TRUE;
