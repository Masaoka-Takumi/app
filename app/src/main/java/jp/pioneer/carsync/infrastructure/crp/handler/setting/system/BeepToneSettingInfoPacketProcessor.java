...
/**
 * BEEP TONE設定情報パケットプロセッサ.
 * <p>
 * BEEP TONE設定情報応答と通知で使用する。
 */
public class BeepToneSettingInfoPacketProcessor {
     ...
     */
    public BeepToneSettingInfoPacketProcessor(@NonNull StatusHolder statusHolder) {
    ...
    public Boolean process(@NonNull IncomingPacket packet) {
        try {
            ...
            // D1:BEEP TONE設定
            setting.beepToneSetting = (ubyteToInt(data[1]) == 0x01);

            setting.updateVersion();
            Timber.d("process() BeepToneSetting = " + setting.beepToneSetting);
            return Boolean.TRUE;
