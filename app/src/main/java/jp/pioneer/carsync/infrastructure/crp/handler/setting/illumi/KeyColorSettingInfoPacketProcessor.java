...
/**
 * KEY COLOR設定情報パケットプロセッサ.
 * <p>
 * KEY COLOR設定情報応答と通知で使用する。
 */
public class KeyColorSettingInfoPacketProcessor {
     ...
     */
    public KeyColorSettingInfoPacketProcessor(@NonNull StatusHolder statusHolder) {
    ...
    public Boolean process(@NonNull IncomingPacket packet) {
        try {
            ...
            // D1:設定色
            illuminationSetting.keyColor = IlluminationColor.valueOf(data[1]);

            illuminationSetting.updateVersion();
            Timber.d("process() KeyColor = " + illuminationSetting.keyColor);
            return Boolean.TRUE;
