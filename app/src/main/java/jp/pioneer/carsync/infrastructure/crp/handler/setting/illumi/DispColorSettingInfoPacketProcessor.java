...
/**
 * DISP COLOR設定情報パケットプロセッサ.
 * <p>
 * DISP COLOR設定情報応答と通知で使用する。
 */
public class DispColorSettingInfoPacketProcessor {
     ...
     */
    public DispColorSettingInfoPacketProcessor(@NonNull StatusHolder statusHolder) {
    ...
    public Boolean process(@NonNull IncomingPacket packet) {
        try {
            ...
            // D1:設定色
            illuminationSetting.dispColor = IlluminationColor.valueOf(data[1]);

            illuminationSetting.updateVersion();
            Timber.d("process() DispColor = " + illuminationSetting.dispColor);
            return Boolean.TRUE;
