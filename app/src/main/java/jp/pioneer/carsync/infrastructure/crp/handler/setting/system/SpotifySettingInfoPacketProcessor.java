...
/**
 * Spotify設定情報パケットプロセッサ.
 * <p>
 * Spotify設定情報応答と通知で使用する。
 */
public class SpotifySettingInfoPacketProcessor {
     ...
     */
    public SpotifySettingInfoPacketProcessor(@NonNull StatusHolder statusHolder) {
    ...
    public Boolean process(@NonNull IncomingPacket packet) {
        try {
            ...
            // D1:Spotify設定
            setting.spotifySetting = (ubyteToInt(data[1]) == 0x01);

            setting.updateVersion();
            Timber.d("process() SpotifySetting = " + setting.spotifySetting);
            return Boolean.TRUE;
