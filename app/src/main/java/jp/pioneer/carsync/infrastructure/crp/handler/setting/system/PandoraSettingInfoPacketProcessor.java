 ...
 * Pandora定情報パケットプロセッサ.
 * <p>
 * Pandora設定情報応答と通知で使用する。
 */
public class PandoraSettingInfoPacketProcessor {
     ...
     */
    public PandoraSettingInfoPacketProcessor(@NonNull StatusHolder statusHolder) {
    ...
    public Boolean process(@NonNull IncomingPacket packet) {
        try {
            ...
            // D1:Pandora設定
            setting.pandoraSetting = (ubyteToInt(data[1]) == 0x01);

            setting.updateVersion();
            Timber.d("process() PandoraSetting = " + setting.pandoraSetting);
            return Boolean.TRUE;
