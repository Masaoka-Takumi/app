...
/**
 * BT PHONE COLOR設定情報パケットプロセッサ.
 * <p>
 * BT PHONE COLOR設定情報応答と通知で使用する。
 */
public class BtPhoneColorSettingInfoPacketProcessor {
     ...
     */
    public BtPhoneColorSettingInfoPacketProcessor(@NonNull StatusHolder statusHolder) {
    ...
    public Boolean process(@NonNull IncomingPacket packet) {
        try {
            ...
            illuminationSetting.btPhoneColor = BtPhoneColor.valueOf(data[1]);

            illuminationSetting.updateVersion();
            Timber.d("process() BtPhoneColor = " + illuminationSetting.btPhoneColor);
            return Boolean.TRUE;
