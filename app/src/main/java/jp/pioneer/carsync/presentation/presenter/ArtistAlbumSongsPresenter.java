...
import static jp.pioneer.carsync.presentation.model.AndroidMusicListType.ARTIST_ALBUM_SONG;

/**
 * アーティスト指定アルバムリスト内楽曲リスト画面のPresenter
 *
 * @see SongsView
 */
@PresenterLifeCycle
public class ArtistAlbumSongsPresenter extends Presenter<SongsView> implements LoaderManager.LoaderCallbacks<Cursor> {
    /** 楽曲リスト種別. */
    private static final AndroidMusicListType CURRENT_LIST_TYPE = ARTIST_ALBUM_SONG;
    ...
    @Inject
    public ArtistAlbumSongsPresenter() {
    }
                ...
                case PUSH:
                    if (current == ListType.ABC_SEARCH_LIST) {
                        ...
                        if (selectPosition < 0) {
                            ...
                            long id = view.getItemId(selectPosition);
                            if (id > 0)
                                onArtistAlbumSongPlayAction(id);
                        }
                    }
                    break;
                case CLOCKWISE:
