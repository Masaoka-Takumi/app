 ...
 */
public final class WeblinkAudioSink implements AudioSink, AudioManager.OnAudioFocusChangeListener {
  ...
  public interface IWLAudioFocusCallback {
  ...
  private final WeblinkAudioTrackPositionTracker audioTrackPositionTracker;
  ...
  private WLAudioStreamEx audioTrack;
  ...
  public WeblinkAudioSink(
    ...
    audioTrackPositionTracker = new WeblinkAudioTrackPositionTracker(new PositionTrackerListener());
  ...
  public void configure(
    ...
    logger.d("configure: [in] encode=%d channel=%d sampleRate=%d specificBuff=%d",
            inputEncoding, inputChannelCount, inputSampleRate, specifiedBufferSize);
    ...
    logger.d("configure: [out] encode=%d channelConfig=%d sampleRate=%d",
            encoding, channelConfig, sampleRate);
    ...
    if (specifiedBufferSize != 0) {
      logger.d("configure: bufferSize: specified");
      bufferSize = specifiedBufferSize;
    } else if (isInputPcm) {
      logger.d("configure: bufferSize: isInputPcm");
      ...
      logger.d("configure: bufferSize: else");
    ...
    logger.d("configure: bufferSize=%d", bufferSize);
  ...
  private void initialize() throws InitializationException {
    ...
    int audioSessionId = audioTrack.getChannelID();
...
//    setVolumeInternal();
  ...
  public boolean handleBuffer(ByteBuffer buffer, long presentationTimeUs)
    ...
    if (audioTrackPositionTracker.isStalled(getWrittenFrames())) {
      logger.w("Resetting stalled audio track");
      reset();
      return true;
    }
...
//  }
  ...
  private void writeBuffer(ByteBuffer buffer, long avSyncPresentationTimeUs) throws WriteException {
    ...
    if ( !playing || !audioTrackPositionTracker.isPlaying() ) {
      return;
    }
...
//    if (Util.SDK_INT < 21) { // isInputPcm == true
//      // Work out how many bytes we can write without the risk of blocking.
      long bytesToWrite = audioTrackPositionTracker.getAvailableBufferSize(writtenPcmBytes);
      if (bytesToWrite < 0) {
        logger.e("getAvailableBufferSize=%d", bytesToWrite);
      }
      if (bytesToWrite > 0) {
        bytesToWrite = Math.min(bytesRemaining, bytesToWrite);
        ...
        {
            // bytesWritten = audioTrack.write(preV21OutputBuffer, preV21OutputBufferOffset, bytesToWrite);
            long presentationTimeUs = getPresentationTimeUs((int) bytesToWrite);
            int writeResult = audioTrack.writeData(preV21OutputBuffer, preV21OutputBufferOffset, (int) bytesToWrite, presentationTimeUs);
...
//    }
  ...
  private long getPresentationTimeUs(int bytesToWrite) {
    long bytes = writtenPcmBytes + bytesToWrite;

    com.abaltatech.weblink.core.audioconfig.AudioFormat format = audioTrack.getAudioFormat();

    int wl_sampleRate = format.getSampleRate();
    int wl_bitsPerChannel = format.getBitsPerChannel();
    int wl_channelCount = format.getChannelCount();

    //logger.v("wl: sampleRate=%d bitsPerChannel=%d channelCount=%d", wl_sampleRate, wl_bitsPerChannel, wl_channelCount);

    // 秒= dataSize[byte] * 8 / (samplingRate[Hz] * 量子化ビット数[bit] * ch数)

    long usec = bytes * 8 * 1000 * 1000/ (wl_sampleRate * wl_bitsPerChannel * wl_channelCount);

    //logger.v("writtenPcmBytes=%d bytesToWrite=%d getPresentationTimeUs=%d", writtenPcmBytes, bytesToWrite, usec);
...
//    }
  ...
  public void reset() {
    ...
    if (isInitialized()) {
      ...
      final WLAudioStreamEx toRelease = audioTrack;
      audioTrack = null;
  ...
  private WLAudioStreamEx initializeAudioTrack() throws InitializationException {
      logger.d("channel=%d sampleRate=%d encode=%d", outputChannelConfig, outputSampleRate, outputEncoding);

      Assertions.checkState(isInputPcm != false);

      WLAudioFormat audioFormat = WLAudioFormat.getDefaultWLAudioFormat();
      audioFormat.setAudioCodec(EAudioCodec.AC_PCM);
      if (outputChannelConfig == AudioFormat.CHANNEL_OUT_STEREO) {
        audioFormat.setChannelCount(2);
      } else if (outputChannelConfig == AudioFormat.CHANNEL_OUT_MONO) {
        audioFormat.setChannelCount(1);
      } else {
        logger.e("Unknown Channel");
        audioFormat.setChannelCount(2);
      }

      audioFormat.setSampleRate(outputSampleRate);         // ex) 44100
      audioFormat.setBitsPerChannel(outputEncoding);     // ex) AudioFormat.ENCODING_PCM_16BIT
      WLAudioStreamEx stream = WLAudioStreamEx.createStream(audioFormat, true, null);

      if (stream == null) {
        logger.e("startAudioStream is failed: ");
      }
  ...
  private final class PositionTrackerListener implements WeblinkAudioTrackPositionTracker.Listener {
    ...
    public void onInvalidLatency(long latencyUs) {
      logger.w("Ignoring impossibly large audio latency: %d", latencyUs);
    }
