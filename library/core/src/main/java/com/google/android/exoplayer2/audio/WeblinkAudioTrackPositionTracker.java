 ...
 * AudioTrack#getPlaybackHeadPosition()} and {AudioTrack#getTimestamp(AudioTimestamp)}.
 *
 * <p>Call {#setAudioTrack(AudioTrack, int, int, int)} to set the audio track to wrap. Call
 ...
 */
/* package */ final class WeblinkAudioTrackPositionTracker {
  ...
  private WeblinkAudioTimestampPoller audioTimestampPoller;
   ...
   */
  public WeblinkAudioTrackPositionTracker(Listener listener) {
  ...
  public void setAudioTrack(
    ...
    audioTimestampPoller = new WeblinkAudioTimestampPoller(audioTrack);
    outputSampleRate = audioTrack.getAudioFormat().getSampleRate();
  ...
  public long getCurrentPositionUs(boolean sourceEnded) {
    if (audioTrack.getPlayState() == WLAudioStreamEx.WL_PLAYSTATE_PLAYING) {
  ...
  public boolean isPlaying() {
    return audioTrack.getPlayState() == WLAudioStreamEx.WL_PLAYSTATE_PLAYING;
  }
  ...
  public boolean mayHandleBuffer(long writtenFrames) {
    if (needsPassthroughWorkarounds) {
      ...
      if (audioTrack.getPlayState() == WLAudioStreamEx.WL_PLAYSTATE_PAUSED) {
      ...
      if (audioTrack.getPlayState() == WLAudioStreamEx.WL_PLAYSTATE_STOPPED && getPlaybackHeadPosition() == 0) {
        return false;
      }
    }
    ...
    if (hadData && !hasData && audioTrack.getPlayState() != WLAudioStreamEx.WL_PLAYSTATE_STOPPED && listener != null) {
   ...
   */
  public long getAvailableBufferSize(long writtenBytes) {
    long bytesPending = (writtenBytes - (getPlaybackHeadPosition() * outputPcmFrameSize));
  ...
  public boolean hasPendingData(long writtenFrames) {
    long pos = getPlaybackHeadPosition();
    boolean force = forceHasPendingData();
    boolean res = (writtenFrames > pos) || force;
  ...
  /**
   * Resets the position tracker. Should be called when the audio track previous passed to {
  ...
  private boolean forceHasPendingData() {
    return needsPassthroughWorkarounds
            && audioTrack.getPlayState() == WLAudioStreamEx.WL_PLAYSTATE_PAUSED
  ...
  private long getPlaybackHeadPosition() {
    ...
    if (audioTrack.getPlayState() == WLAudioStreamEx.WL_PLAYSTATE_STOPPED) {
      // The audio track hasn't been started.
      return 0;
    }
    long rawPlaybackHeadPosition = audioTrack.getPlaybackPositionUs() * audioTrack.getAudioFormat().getSampleRate() / 1000 / 1000; // Frameへ変換

    if (needsPassthroughWorkarounds) {
      // Work around an issue with passthrough/direct AudioTracks on platform API versions 21/22
      // where the playback head position jumps back to zero on paused passthrough/direct audio
      // tracks. See [Internal: b/19187573].
      if (audioTrack.getPlayState() == WLAudioStreamEx.WL_PLAYSTATE_PAUSED && rawPlaybackHeadPosition == 0) {
    ...
    if (Util.SDK_INT <= 28) {
      if (rawPlaybackHeadPosition == 0
              && lastRawPlaybackHeadPosition > 0
              && audioTrack.getPlayState() == WLAudioStreamEx.WL_PLAYSTATE_PLAYING) {
